...
class DialogAddMeal : BaseDialogFragment(), DatePickerDialog.OnDateSetListener {
  ...
  override fun onViewStateRestored(savedInstanceState: Bundle?) {
    ...
    if (savedInstanceState != null) {
      if (!savedInstanceState.getString("title").isNullOrEmpty()) {
        title.editText?.setText(savedInstanceState.getString("title"))
      }

      if (!savedInstanceState.getString("description").isNullOrEmpty()) {
        description.editText?.setText(savedInstanceState.getString("description"))
      }

      if (!savedInstanceState.getString("date").isNullOrEmpty()) {
        dateEditText.editText?.setText(
            getLongFormattedDate(savedInstanceState.getSerializable("date") as Date)
        )
      }
    }
  }
